...
export function CxSoundwaveCard({ scope, className }: CxSoundwaveCardProps) {
  const [range, setRange] = useState<'7d' | '30d' | '90d'>('30d');
  const [activeSkillId, setActiveSkillId] = useState<CxSkillId | null>(null);

  const series = useMemo(() => {
    const days = range === '7d' ? 7 : range === '30d' ? 30 : 90;
    return rollupCxTrend(scope, days);
  }, [scope, range]);

  const mode = scope.role === 'owner' && !scope.storeId ? 'groupOnly' : 'compare';
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        <div className="space-y-1">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="bg-slate-900 border-white/10 text-xs max-w-[200px]">
                  ...
                  Neon lines show {getScopeLabel(scope).toLowerCase()} performance.
          ...
          <CardDescription className="text-white/40 text-xs">
            {getScopeLabel(scope)} trends over {range === '7d' ? 'the last week' : range === '30d' ? '30 days' : '90 days'}.
          </CardDescription>
        </div>
