...
import {
  DECLINED_WORK_ATTITUDE,
  DECLINED_WORK_TIMING,
  DECLINED_WORK_TYPES,
  DECLINED_WORK_URGENCY,
  getAutoDriveCxDeclinedWorkRecoveryEnhancement,
  getDeclinedWorkRecoveryPlan,
  getSprocketDeclinedWorkRecoveryEnhancement,
  type DeclinedWorkAttitude,
  type DeclinedWorkRecoveryInput,
  type DeclinedWorkRecoverySavedScenario,
  type DeclinedWorkTiming,
  type DeclinedWorkType,
  type DeclinedWorkUrgency,
} from '@/lib/tools/declined-work-recovery';

const TOOL_ID = 'declined-work-recovery';
const LOCAL_SCENARIOS_KEY = 'declinedWorkRecoverySavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): DeclinedWorkRecoverySavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as DeclinedWorkRecoverySavedScenario[];
...
function writeLocalScenarios(scenarios: DeclinedWorkRecoverySavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: DeclinedWorkRecoveryInput, summary: string): string {
  return [
    'DECLINED WORK RECOVERY',
    '',
    `Declined Work Type: ${input.workType}`,
    `Time Since Decline: ${input.timeSinceDecline}`,
    `Urgency: ${input.urgency}`,
    `Customer Attitude: ${input.customerAttitude}`,
    '',
    summary,
  ].join('\n');
}

export default function DeclinedWorkRecoveryPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [workType, setWorkType] = useState<DeclinedWorkType>('maintenance');
  const [timeSinceDecline, setTimeSinceDecline] = useState<DeclinedWorkTiming>('1 to 7 days');
  const [urgency, setUrgency] = useState<DeclinedWorkUrgency>('medium');
  const [customerAttitude, setCustomerAttitude] = useState<DeclinedWorkAttitude>('price-sensitive');
  const [savedScenarios, setSavedScenarios] = useState<DeclinedWorkRecoverySavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketDeclinedWorkRecoveryEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxDeclinedWorkRecoveryEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [workType, timeSinceDecline, urgency, customerAttitude]);

  const input = useMemo<DeclinedWorkRecoveryInput>(() => ({
    workType,
    timeSinceDecline,
    urgency,
    customerAttitude,
  ...
  const plan = useMemo(() => getDeclinedWorkRecoveryPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Best Re-Entry Angle] ${plan.bestReEntryAngle}`,
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: DeclinedWorkRecoverySavedScenario = {
      ...
      urgency,
      customerAttitude,
      bestReEntryAngle: plan.bestReEntryAngle,
  ...
  }, [customerAttitude, plan.askThis, plan.bestReEntryAngle, plan.doNotDoThis, plan.sayThis, plan.whyNow, savedScenarios, timeSinceDecline, toast, urgency, workType]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync declined-work scenarios.')) return;
    if (!firebaseUser) {
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Best Re-Entry Angle: ${plan.bestReEntryAngle}\nAsk This: ${plan.askThis}`),
    });
  ...
  }, [firebaseUser, input, plan.askThis, plan.bestReEntryAngle, requireFeature, toast]);

  const handleRunSprocket = useCallback(() => {
    if (!requireFeature(FEATURES.SPROCKET, 'Unlock Sprocket for deeper recovery coaching.')) return;
    setSprocketOutput(getSprocketDeclinedWorkRecoveryEnhancement(input, plan));
  ...
  const handleRunAutoDrive = useCallback(() => {
    ...
    setCxOutput(getAutoDriveCxDeclinedWorkRecoveryEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[DeclinedWorkRecovery] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Declined Work Recovery</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Revisit declined work with a low-pressure, customer-friendly approach that improves re-approval conversations.
          </p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">1. Declined Work Type</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {DECLINED_WORK_TYPES.map((item) => (
              <ChipButton key={item} active={workType === item} label={item} onClick={() => withUsageTracking(() => setWorkType(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">2. Time Since Decline</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {DECLINED_WORK_TIMING.map((item) => (
              <ChipButton key={item} active={timeSinceDecline === item} label={item} onClick={() => withUsageTracking(() => setTimeSinceDecline(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">3. Urgency</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {DECLINED_WORK_URGENCY.map((item) => (
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">4. Customer Attitude</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {DECLINED_WORK_ATTITUDE.map((item) => (
              <ChipButton key={item} active={customerAttitude === item} label={item} onClick={() => withUsageTracking(() => setCustomerAttitude(item))} />
            ))}
          </CardContent>
        </Card>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Recovery Plan</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Re-Entry Angle</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestReEntryAngle}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Why Now</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.whyNow}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Stronger recovery framing and delivery coaching.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">Likely reason declined:</span> {sprocketOutput.likelyDeclineReason}</p>
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">Stronger framing:</span> {sprocketOutput.strongerRecoveryFraming}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware declined-work recovery adaptation.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.workType} - {scenario.timeSinceDecline}</p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.bestReEntryAngle}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks skill-aware declined-work intelligence.' : undefined)}
        onOpenChange={(open) => {
