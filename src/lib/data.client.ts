...
export async function getCombinedTeamData(dealershipId: string, user: User): Promise<any> {
    ...
    const logsByUserId = new Map<string, LessonLog[]>();
    const activityByMember = await Promise.all(
        filtered.map(async (member) => {
            const logs = await getConsultantActivity(member.userId);
            logsByUserId.set(member.userId, logs);
            return logs;
        })
    );

    const teamActivity = filtered.map((member, index) => (
        buildTeamActivityRow(member, activityByMember[index] || [])
    ));
