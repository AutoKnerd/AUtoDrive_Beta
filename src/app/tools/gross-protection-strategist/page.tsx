...
import {
  ...
  GROSS_MANAGER_GOALS,
  GROSS_OBJECTION_TYPES,
  getAutoDriveCxGrossProtectionEnhancement,
  getGrossProtectionPlan,
  getSprocketGrossProtectionEnhancement,
  type GrossCustomerLeverageLevel,
  type GrossDealPressureLevel,
  type GrossManagerGoal,
  type GrossObjectionType,
  type GrossProtectionInput,
  type GrossProtectionSavedScenario,
} from '@/lib/tools/gross-protection-strategist';

const TOOL_ID = 'gross-protection-strategist';
const LOCAL_SCENARIOS_KEY = 'grossProtectionStrategistSavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): GrossProtectionSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as GrossProtectionSavedScenario[];
...
function writeLocalScenarios(scenarios: GrossProtectionSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: GrossProtectionInput, summary: string): string {
  return [
    'GROSS PROTECTION STRATEGIST',
    '',
    `Deal Pressure: ${input.dealPressure}`,
    `Objection Type: ${input.objectionType}`,
    `Customer Leverage: ${input.customerLeverage}`,
    `Manager Goal: ${input.managerGoal}`,
    '',
    summary,
  ].join('\n');
}

export default function GrossProtectionStrategistPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [dealPressure, setDealPressure] = useState<GrossDealPressureLevel>('moderate');
  const [objectionType, setObjectionType] = useState<GrossObjectionType>('price');
  const [customerLeverage, setCustomerLeverage] = useState<GrossCustomerLeverageLevel>('moderate');
  const [managerGoal, setManagerGoal] = useState<GrossManagerGoal>('hold gross');
  const [savedScenarios, setSavedScenarios] = useState<GrossProtectionSavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketGrossProtectionEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxGrossProtectionEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [dealPressure, objectionType, customerLeverage, managerGoal]);

  const input = useMemo<GrossProtectionInput>(() => ({
    dealPressure,
    objectionType,
    customerLeverage,
    managerGoal,
  }), [customerLeverage, dealPressure, managerGoal, objectionType]);

  const plan = useMemo(() => getGrossProtectionPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Best Gross-Protection Angle] ${plan.bestGrossProtectionAngle}`,
      `[Say This] ${plan.sayThis}`,
      `[Best Concession Strategy] ${plan.bestConcessionStrategy}`,
      `[Line Not To Cross] ${plan.lineNotToCross}`,
      `[Do Not Do This] ${plan.doNotDoThis}`,
    ].join('\n\n');

    try {
      await navigator.clipboard.writeText(payload);
      toast({ title: 'Copied', description: 'Gross strategy copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: GrossProtectionSavedScenario = {
      ...
      customerLeverage,
      managerGoal,
      bestGrossProtectionAngle: plan.bestGrossProtectionAngle,
      sayThis: plan.sayThis,
      bestConcessionStrategy: plan.bestConcessionStrategy,
      lineNotToCross: plan.lineNotToCross,
  ...
  }, [customerLeverage, dealPressure, managerGoal, objectionType, plan.bestConcessionStrategy, plan.bestGrossProtectionAngle, plan.doNotDoThis, plan.lineNotToCross, plan.sayThis, savedScenarios, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync gross-protection scenarios.')) return;
    if (!firebaseUser) {
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Best Gross-Protection Angle: ${plan.bestGrossProtectionAngle}\nBest Concession Strategy: ${plan.bestConcessionStrategy}`),
    });
  ...
  }, [firebaseUser, input, plan.bestConcessionStrategy, plan.bestGrossProtectionAngle, requireFeature, toast]);
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized negotiation adaptation.')) return;
    setCxOutput(getAutoDriveCxGrossProtectionEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[GrossProtectionStrategist] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Gross Protection Strategist</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Choose the smartest gross-protection strategy for live deals without damaging trust or momentum.
          </p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">1. Deal Pressure</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {GROSS_DEAL_PRESSURE_LEVELS.map((item) => (
              <ChipButton key={item} active={dealPressure === item} label={item} onClick={() => withUsageTracking(() => setDealPressure(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">2. Customer Objection Type</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {GROSS_OBJECTION_TYPES.map((item) => (
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">3. Customer Leverage</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-3 gap-2">
            {GROSS_CUSTOMER_LEVERAGE_LEVELS.map((item) => (
              <ChipButton key={item} active={customerLeverage === item} label={item} onClick={() => withUsageTracking(() => setCustomerLeverage(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">4. Manager Goal</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {GROSS_MANAGER_GOALS.map((item) => (
              <ChipButton key={item} active={managerGoal === item} label={item} onClick={() => withUsageTracking(() => setManagerGoal(item))} />
            ))}
          </CardContent>
        </Card>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Gross Strategy</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Gross-Protection Angle</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestGrossProtectionAngle}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Concession Strategy</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestConcessionStrategy}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#ffd08a]">Line Not To Cross</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#ffebb9]">{plan.lineNotToCross}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Deeper margin-hold diagnosis and delivery coaching.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">Likely value breakdown:</span> {sprocketOutput.likelyValueBreakdown}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Better hold strategy:</span> {sprocketOutput.betterHoldStrategy}</p>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware negotiation adaptation for managers.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.objectionType} - {scenario.dealPressure}</p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.bestGrossProtectionAngle}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks personalized gross-protection adaptation.' : undefined)}
        onOpenChange={(open) => {
