...
import {
  ...
  getAutoDriveCxCommitmentLadderEnhancement,
  getCommitmentLadderPlan,
  getSprocketCommitmentLadderEnhancement,
  type CommitmentConcern,
  type CommitmentDealMoment,
  type CommitmentLadderInput,
  type CommitmentLadderSavedScenario,
  type CommitmentOpenness,
  type CommitmentResistance,
} from '@/lib/tools/commitment-ladder';

const TOOL_ID = 'commitment-ladder';
const LOCAL_SCENARIOS_KEY = 'commitmentLadderSavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): CommitmentLadderSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as CommitmentLadderSavedScenario[];
...
function writeLocalScenarios(scenarios: CommitmentLadderSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: CommitmentLadderInput, summary: string): string {
  return [
    'COMMITMENT LADDER',
    '',
    `Deal Moment: ${input.dealMoment}`,
    `Customer Openness: ${input.customerOpenness}`,
    `Resistance Level: ${input.resistanceLevel}`,
    `Concern Type: ${input.concernType || 'unclear'}`,
    '',
    summary,
  ].join('\n');
}

export default function CommitmentLadderPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [dealMoment, setDealMoment] = useState<CommitmentDealMoment>('after needs assessment');
  const [customerOpenness, setCustomerOpenness] = useState<CommitmentOpenness>('neutral');
  const [resistanceLevel, setResistanceLevel] = useState<CommitmentResistance>('moderate');
  const [concernType, setConcernType] = useState<CommitmentConcern>('unclear');
  const [savedScenarios, setSavedScenarios] = useState<CommitmentLadderSavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketCommitmentLadderEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxCommitmentLadderEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    if (handoff?.draft) {
      const maybeConcern = COMMITMENT_CONCERNS.find((item) => handoff.draft?.toLowerCase().includes(item.toLowerCase()));
      if (maybeConcern) setConcernType(maybeConcern);
    }
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [dealMoment, customerOpenness, resistanceLevel, concernType]);

  const input = useMemo<CommitmentLadderInput>(() => ({
    dealMoment,
    customerOpenness,
    resistanceLevel,
    concernType,
  ...
  const plan = useMemo(() => getCommitmentLadderPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Best Next Commitment] ${plan.bestNextCommitment}`,
      `[Say This] ${plan.sayThis}`,
      `[Ask This] ${plan.askThis}`,
      `[What This Unlocks] ${plan.whatThisUnlocks}`,
      `[Do Not Do This] ${plan.doNotDoThis}`,
    ].join('\n\n');

    try {
      await navigator.clipboard.writeText(payload);
      toast({ title: 'Copied', description: 'Commitment plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: CommitmentLadderSavedScenario = {
      ...
      resistanceLevel,
      concernType,
      bestNextCommitment: plan.bestNextCommitment,
      sayThis: plan.sayThis,
      askThis: plan.askThis,
      whatThisUnlocks: plan.whatThisUnlocks,
  ...
  }, [concernType, customerOpenness, dealMoment, plan.askThis, plan.bestNextCommitment, plan.doNotDoThis, plan.sayThis, plan.whatThisUnlocks, resistanceLevel, savedScenarios, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync commitment scenarios.')) return;
    if (!firebaseUser) {
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Best Next Commitment: ${plan.bestNextCommitment}\nAsk This: ${plan.askThis}`),
    });
  ...
  }, [firebaseUser, input, plan.askThis, plan.bestNextCommitment, requireFeature, toast]);
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized commitment guidance.')) return;
    setCxOutput(getAutoDriveCxCommitmentLadderEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[CommitmentLadder] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Commitment Ladder</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Choose the right next commitment step without pushing too far, too soon.
          </p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">1. Deal Moment</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Pick where you are in the deal.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {COMMITMENT_DEAL_MOMENTS.map((item) => (
              <ChipButton key={item} active={dealMoment === item} label={item} onClick={() => withUsageTracking(() => setDealMoment(item))} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">2. Openness and Resistance</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Tune the ask to current customer posture.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#84a0c4]">Customer Openness</p>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {COMMITMENT_OPENNESS.map((item) => (
                  <ChipButton key={item} active={customerOpenness === item} label={item} onClick={() => withUsageTracking(() => setCustomerOpenness(item))} />
                ))}
              </div>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#84a0c4]">Resistance Level</p>
              <div className="grid grid-cols-3 gap-2">
                {COMMITMENT_RESISTANCE.map((item) => (
                  <ChipButton key={item} active={resistanceLevel === item} label={item} onClick={() => withUsageTracking(() => setResistanceLevel(item))} />
                ))}
              </div>
            </div>
          </CardContent>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">3. Optional Concern</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Select the dominant concern if known.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {COMMITMENT_CONCERNS.map((item) => (
              <ChipButton key={item} active={concernType === item} label={item} onClick={() => withUsageTracking(() => setConcernType(item))} />
            ))}
          </CardContent>
        </Card>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Next Commitment Step</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Next Commitment</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestNextCommitment}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">What This Unlocks</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.whatThisUnlocks}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Sharper commitment asks and delivery coaching.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">Why commitment is stalling:</span> {sprocketOutput.stallDiagnosis}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Sharper ask:</span> {sprocketOutput.sharperAsk}</p>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware commitment pacing and tone adaptation.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
            ...
            <CardContent className="space-y-3">
              ...
              {cxOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">Skill-aware adjustment:</span> {cxOutput.adaptedStep}</p>
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.dealMoment} - {scenario.customerOpenness}</p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.bestNextCommitment}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks skill-aware commitment intelligence.' : undefined)}
        onOpenChange={(open) => {
