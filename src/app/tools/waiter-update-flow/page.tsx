...
import {
  ...
  WAITER_LANE_STAGES,
  WAITER_RESET_LEVELS,
  getAutoDriveCxWaiterUpdateEnhancement,
  getSprocketWaiterUpdateEnhancement,
  getWaiterUpdatePlan,
  type WaiterCustomerMood,
  type WaiterLaneStage,
  type WaiterResetLevel,
  type WaiterUpdateInput,
  type WaiterUpdateSavedScenario,
} from '@/lib/tools/waiter-update-flow';

const TOOL_ID = 'waiter-update-flow';
const LOCAL_SCENARIOS_KEY = 'waiterUpdateFlowSavedScenariosV1';
...
function readLocalScenarios(): WaiterUpdateSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as WaiterUpdateSavedScenario[];
...
function writeLocalScenarios(scenarios: WaiterUpdateSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: WaiterUpdateInput, summary: string): string {
  return [
    'WAITER UPDATE FLOW',
    '',
    `Delay Tension: ${input.delayTension}`,
    `Expectation Reset Level: ${input.expectationResetLevel}`,
    `Patience Level: ${input.patienceLevel}`,
    `Service Lane Stage: ${input.laneStage}`,
...
function laneStagePosition(stage: WaiterLaneStage): number {
  const index = WAITER_LANE_STAGES.indexOf(stage);
  if (index <= 0) return 0;
  return (index / (WAITER_LANE_STAGES.length - 1)) * 100;
}

export default function WaiterUpdateFlowPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [delayTension, setDelayTension] = useState(52);
  const [expectationResetLevel, setExpectationResetLevel] = useState<WaiterResetLevel>('moderate');
  const [patienceLevel, setPatienceLevel] = useState(60);
  const [laneStage, setLaneStage] = useState<WaiterLaneStage>('inspection in progress');
  const [customerMood, setCustomerMood] = useState<WaiterCustomerMood | null>('calm');
  const [savedScenarios, setSavedScenarios] = useState<WaiterUpdateSavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketWaiterUpdateEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxWaiterUpdateEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [delayTension, expectationResetLevel, patienceLevel, laneStage, customerMood]);

  const input = useMemo<WaiterUpdateInput>(() => ({
    delayTension,
    expectationResetLevel,
    patienceLevel,
    laneStage,
    customerMood,
  }), [delayTension, expectationResetLevel, patienceLevel, laneStage, customerMood]);

  const plan = useMemo(() => getWaiterUpdatePlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      ...
      `[When To Update Again] ${plan.whenToUpdateAgain}`,
      `[How To Reset Expectations] ${plan.howToResetExpectations}`,
      `[Do Not Say This] ${plan.doNotSayThis}`,
    ].join('\n\n');
    try {
      await navigator.clipboard.writeText(payload);
      toast({ title: 'Copied', description: 'Waiter update plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: WaiterUpdateSavedScenario = {
      ...
      whenToUpdateAgain: plan.whenToUpdateAgain,
      howToResetExpectations: plan.howToResetExpectations,
  ...
  }, [delayTension, expectationResetLevel, patienceLevel, laneStage, customerMood, plan, savedScenarios, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync waiter update scenarios.')) return;
    if (!firebaseUser) {
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized waiting-customer adaptation.')) return;
    setCxOutput(getAutoDriveCxWaiterUpdateEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[WaiterUpdateFlow] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Waiter Update Flow</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Communicate clearer live updates to waiting customers and reset expectations before frustration spikes.
          </p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">Delay Tension Gauge</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Estimate current frustration pressure in the lane.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between text-sm text-[#c8d8f1]">
              <span>Low</span>
              <span className="font-semibold text-[#eff7ff]">{delayTension}</span>
              <span>High</span>
            </div>
            <input
              type="range"
              ...
              max={100}
              value={delayTension}
              onChange={(event) => withUsageTracking(() => setDelayTension(clamp(Number(event.target.value))))}
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">Expectation-Reset Toggle</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Pick how strongly to reset timeline expectations.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-3 gap-2">
            {WAITER_RESET_LEVELS.map((level) => (
              <ChipButton key={level} active={expectationResetLevel === level} label={level} onClick={() => withUsageTracking(() => setExpectationResetLevel(level))} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">Customer Patience Timer</CardTitle>
            <CardDescription className="text-[#9cb0cd]">How much patience remains right now.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            <div className="flex items-center justify-between text-sm text-[#c8d8f1]">
              <span>Low</span>
              <span className="font-semibold text-[#eff7ff]">{patienceLevel}</span>
              <span>High</span>
            </div>
            <input
              type="range"
              ...
              max={100}
              value={patienceLevel}
              onChange={(event) => withUsageTracking(() => setPatienceLevel(clamp(Number(event.target.value))))}
              className="h-2 w-full cursor-pointer appearance-none rounded-full bg-[#1a304c]"
            />
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">Service Lane Flow Strip</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Show where the visit is currently stuck.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="relative h-8 rounded-full border border-[#355274] bg-[#0f233c]">
              ...
              <div
                ...
                style={{ left: `calc(${laneStagePosition(laneStage)}% - 8px)` }}
              />
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {WAITER_LANE_STAGES.map((stage) => (
                <ChipButton key={stage} active={laneStage === stage} label={stage} onClick={() => withUsageTracking(() => setLaneStage(stage))} />
              ))}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">Optional Customer Mood</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-5">
            <ChipButton active={customerMood === null} label="Not set" onClick={() => withUsageTracking(() => setCustomerMood(null))} />
            {WAITER_CUSTOMER_MOODS.map((mood) => (
              <ChipButton key={mood} active={customerMood === mood} label={mood} onClick={() => withUsageTracking(() => setCustomerMood(mood))} />
            ))}
          </CardContent>
        </Card>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Update Strategy</h2>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Update Approach</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestUpdateApproach}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Say This Now</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.sayThisNow}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">When To Update Again</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.whenToUpdateAgain}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">How To Reset Expectations</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.howToResetExpectations}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Better tone, stronger reset, and reaction prep.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">Stronger expectation reset:</span> {sprocketOutput.strongerExpectationReset}</p>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware service-lane communication adaptation.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.laneStage} - tension {scenario.delayTension}</p>
                    <Button
                      size="sm"
                      variant="ghost"
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks personalized service-lane communication adaptation.' : undefined)}
        onOpenChange={(open) => {
