Create a single horizontal sprite strip for the Codex app digital pet `patchy` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Supportive, analytical, organized, quietly enthusiastic, lives in the terminal..
