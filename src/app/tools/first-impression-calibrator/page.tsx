...
import {
  ...
  FIRST_IMPRESSION_SETTINGS,
  getAutoDriveCxFirstImpressionEnhancement,
  getFirstImpressionPlan,
  getSprocketFirstImpressionEnhancement,
  type FirstImpressionComfortRead,
  type FirstImpressionInput,
  type FirstImpressionPace,
  type FirstImpressionSavedScenario,
  type FirstImpressionSetting,
} from '@/lib/tools/first-impression-calibrator';

const TOOL_ID = 'first-impression-calibrator';
const LOCAL_SCENARIOS_KEY = 'firstImpressionCalibratorSavedScenariosV1';
...
function readLocalScenarios(): FirstImpressionSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as FirstImpressionSavedScenario[];
...
function writeLocalScenarios(scenarios: FirstImpressionSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: FirstImpressionInput, summary: string): string {
  return [
    'FIRST IMPRESSION CALIBRATOR',
    '',
    `Customer Energy: ${input.customerEnergy}`,
    `Comfort Read: ${input.comfortRead}`,
    `Warmth Level: ${input.warmthLevel}`,
    `Pace: ${input.pace}`,
    `Setting: ${input.setting ?? 'not set'}`,
    '',
    summary,
  ].join('\n');
}

function energyRing(energy: number): string {
  if (energy >= 75) return 'from-[#38f8a8] via-[#31dcb0] to-[#1fa3c6]';
  if (energy >= 45) return 'from-[#67d0ff] via-[#4ba8ff] to-[#2b7dce]';
  return 'from-[#a5b7d4] via-[#6c8bb4] to-[#446084]';
}

export default function FirstImpressionCalibratorPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [customerEnergy, setCustomerEnergy] = useState(56);
  const [comfortRead, setComfortRead] = useState<FirstImpressionComfortRead>('guarded');
  const [warmthLevel, setWarmthLevel] = useState(58);
  const [pace, setPace] = useState<FirstImpressionPace>('balanced');
  const [setting, setSetting] = useState<FirstImpressionSetting | null>('showroom walk-in');
  const [savedScenarios, setSavedScenarios] = useState<FirstImpressionSavedScenario[]>([]);
  ...
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxFirstImpressionEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [customerEnergy, comfortRead, warmthLevel, pace, setting]);

  const input = useMemo<FirstImpressionInput>(() => ({
    customerEnergy,
    comfortRead,
    warmthLevel,
    pace,
  ...
  const plan = useMemo(() => getFirstImpressionPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Best Opening Style] ${plan.bestOpeningStyle}`,
      `[Say This First] ${plan.sayThisFirst}`,
      `[Ask This First] ${plan.askThisFirst}`,
      `[Tone Guidance] ${plan.toneGuidance}`,
      `[Do Not Do This] ${plan.doNotDoThis}`,
    ].join('\n\n');
    try {
      await navigator.clipboard.writeText(payload);
      toast({ title: 'Copied', description: 'Opening plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: FirstImpressionSavedScenario = {
      ...
      pace,
      setting,
      bestOpeningStyle: plan.bestOpeningStyle,
      sayThisFirst: plan.sayThisFirst,
      askThisFirst: plan.askThisFirst,
  ...
  }, [customerEnergy, comfortRead, warmthLevel, pace, setting, plan, savedScenarios, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync first-impression scenarios.')) return;
    if (!firebaseUser) {
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Best Opening Style: ${plan.bestOpeningStyle}\nSay This First: ${plan.sayThisFirst}`),
    });
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized first-minute coaching.')) return;
    setCxOutput(getAutoDriveCxFirstImpressionEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[FirstImpressionCalibrator] unlock capture failed:', captureResult.message);
    }
  ...
  const ringDegrees = Math.round((clamp(customerEnergy) / 100) * 360);
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">First Impression Calibrator</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Calibrate your first 2 minutes so the customer feels welcomed, comfortable, and understood.
          </p>
        </section>
        ...
        <div className="grid gap-5 md:grid-cols-[1.05fr_1fr]">
          <Card className="border-[#2a3f5f] bg-[#0f1b30]">
            <CardHeader>
              <CardTitle className="text-lg text-[#eff6ff]">Calibrate the Moment</CardTitle>
              <CardDescription className="text-[#9cb2cf]">Map comfort, warmth, and pace using quick tactile controls.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <p className="text-xs uppercase tracking-[0.2em] text-[#7fa7cf]">Customer Energy Dial</p>
                <div className="flex items-center gap-4 rounded-xl border border-[#294665] bg-[#0f2238] p-3">
                  <div
                    className={`grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br ${energyRing(customerEnergy)}`}
                  ...
                  <div className="flex-1 space-y-2">
                    <input
                      ...
                      min={0}
                      max={100}
                      value={customerEnergy}
                      onChange={(event) => withUsageTracking(() => setCustomerEnergy(clamp(Number(event.target.value))))}
                    ...
                    <div className="h-1.5 rounded-full bg-[#10243a]">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-[#2e90c8] to-[#35f3b2]"
                        style={{ width: `${Math.round((ringDegrees / 360) * 100)}%` }}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <p className="text-xs uppercase tracking-[0.2em] text-[#7fa7cf]">Consultant Warmth</p>
                <div className="rounded-xl border border-[#294665] bg-[#0f2238] p-3">
                  <input
                    ...
                    max={100}
                    value={warmthLevel}
                    onChange={(event) => withUsageTracking(() => setWarmthLevel(clamp(Number(event.target.value))))}
                    className="w-full accent-[#00d8e5]"
                  />
                  <div className="mt-2 flex justify-between text-xs text-[#9ab4d4]">
                    <span>Reserved</span>
                    <span className="font-semibold text-[#d9eeff]">{warmthLevel}</span>
                    <span>Warm</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-xs uppercase tracking-[0.2em] text-[#7fa7cf]">Pace Preference</p>
                <div className="grid grid-cols-3 gap-2">
                  {FIRST_IMPRESSION_PACE.map((option) => (
                    <ToggleButton
                      key={option}
                      active={pace === option}
                      label={option[0].toUpperCase() + option.slice(1)}
                      onClick={() => withUsageTracking(() => setPace(option))}
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-xs uppercase tracking-[0.2em] text-[#7fa7cf]">Comfort Read</p>
                <div className="grid grid-cols-2 gap-2">
                  {FIRST_IMPRESSION_COMFORT_READS.map((option) => (
                    <ToggleButton
                      key={option}
                      active={comfortRead === option}
                      label={option[0].toUpperCase() + option.slice(1)}
                      onClick={() => withUsageTracking(() => setComfortRead(option))}
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-xs uppercase tracking-[0.2em] text-[#7fa7cf]">Arrival Setting (Optional)</p>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {FIRST_IMPRESSION_SETTINGS.map((option) => (
                    <ToggleButton
                      key={option}
                      active={setting === option}
                      label={option[0].toUpperCase() + option.slice(1)}
                      onClick={() => withUsageTracking(() => setSetting(option))}
                    />
                  ))}
                </div>
              </div>
          ...
          <Card className="border-[#2a3f5f] bg-[#0f1b30]">
            <CardHeader>
              <CardTitle className="text-lg text-[#f4fbff]">Opening Plan</CardTitle>
              <CardDescription className="text-[#9cb2cf]">Actionable in under 10 seconds.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="rounded-xl border border-[#2b4d6f] bg-[#10253d] p-3">
                <p className="text-xs uppercase tracking-[0.14em] text-[#7de9ff]">Best Opening Style</p>
                <p className="mt-1 text-sm font-semibold text-[#e6f3ff]">{plan.bestOpeningStyle}</p>
              </div>
              <div className="rounded-xl border border-[#2b4d6f] bg-[#10253d] p-3">
                <p className="text-xs uppercase tracking-[0.14em] text-[#7de9ff]">Say This First</p>
                <p className="mt-1 text-sm text-[#d5e9ff]">{plan.sayThisFirst}</p>
              </div>
              <div className="rounded-xl border border-[#2b4d6f] bg-[#10253d] p-3">
                <p className="text-xs uppercase tracking-[0.14em] text-[#7de9ff]">Ask This First</p>
                <p className="mt-1 text-sm text-[#d5e9ff]">{plan.askThisFirst}</p>
              </div>
              <div className="rounded-xl border border-[#2b4d6f] bg-[#10253d] p-3">
                <p className="text-xs uppercase tracking-[0.14em] text-[#7de9ff]">Tone Guidance</p>
                <p className="mt-1 text-sm text-[#d5e9ff]">{plan.toneGuidance}</p>
              </div>
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Deeper first-minute diagnosis and rewrite support.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">Likely reason:</span> {sprocketOutput.likelyReason}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Sharper opening:</span> {sprocketOutput.sharperOpeningLine}</p>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware first-minute calibration.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
                ...
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">
                      {scenario.comfortRead[0].toUpperCase() + scenario.comfortRead.slice(1)} · {scenario.pace[0].toUpperCase() + scenario.pace.slice(1)} pace
                    </p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.bestOpeningStyle}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks skill-aware opening guidance.' : undefined)}
        onOpenChange={(open) => {
