...
import {
  ...
  RECOVERY_GOALS,
  RECOVERY_SITUATIONS,
  getAutoDriveCxDealRecoveryEnhancement,
  getDealRecoveryPlan,
  getSprocketDealRecoveryEnhancement,
  type CustomerMindset,
  type CustomerStatus,
  type DealRecoveryInput,
  type DealRecoverySavedScenario,
  type RecoveryGoal,
  type RecoverySituation,
} from '@/lib/tools/deal-recovery-planner';

const TOOL_ID = 'deal-recovery';
const LOCAL_SCENARIOS_KEY = 'dealRecoverySavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): DealRecoverySavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as DealRecoverySavedScenario[];
...
function writeLocalScenarios(scenarios: DealRecoverySavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: DealRecoveryInput, summary: string): string {
  return [
    'DEAL RECOVERY PLANNER',
    '',
    `Situation: ${input.situation}`,
    `Customer Status: ${input.customerStatus}`,
    `Mindset: ${input.customerMindset || 'Not set'}`,
    `Recovery Goal: ${input.recoveryGoal || 'Not set'}`,
...
export default function DealRecoveryPage() {
  ...
  const [situation, setSituation] = useState<RecoverySituation>('deal feels stuck without a clear objection');
  const [customerStatus, setCustomerStatus] = useState<CustomerStatus>('left the store today');
  const [customerMindset, setCustomerMindset] = useState<CustomerMindset>('hard to read');
  const [recoveryGoal, setRecoveryGoal] = useState<RecoveryGoal>('reopen the conversation');
  const [context, setContext] = useState('');

  const [savedScenarios, setSavedScenarios] = useState<DealRecoverySavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketDealRecoveryEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxDealRecoveryEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [situation, customerStatus, customerMindset, recoveryGoal, context]);

  const input = useMemo<DealRecoveryInput>(() => ({
    situation,
    customerStatus,
    customerMindset,
    recoveryGoal,
  ...
  const plan = useMemo(() => getDealRecoveryPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Likely Breakdown] ${plan.likelyBreakdown}`,
      `[Best Recovery Path] ${plan.bestRecoveryPath}`,
    ...
    try {
      ...
      toast({ title: 'Copied', description: 'Recovery plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: DealRecoverySavedScenario = {
      ...
      customerMindset,
      recoveryGoal,
      likelyBreakdown: plan.likelyBreakdown,
      bestRecoveryPath: plan.bestRecoveryPath,
  ...
  }, [customerMindset, customerStatus, plan.bestRecoveryPath, plan.doNotDoThis, plan.likelyBreakdown, plan.nextMove, plan.sayThis, recoveryGoal, savedScenarios, situation, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync recovery scenarios.')) return;
    if (!firebaseUser) {
      toast({ variant: 'destructive', title: 'Sign in required', description: 'Sign in to save this plan.' });
      return;
    }
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Next Move: ${plan.nextMove}\nSay This: ${plan.sayThis}`),
    });
  ...
  }, [firebaseUser, input, plan.nextMove, plan.sayThis, requireFeature, toast]);
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized recovery guidance.')) return;
    setCxOutput(getAutoDriveCxDealRecoveryEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[DealRecovery] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Deal Recovery Planner</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Diagnose stalled deals fast, pick the smartest re-entry move, and recover momentum with confidence.
          </p>
          <p className="text-xs uppercase tracking-[0.12em] text-[#6f89af]">Clarity first, pressure second, momentum rebuilt</p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">1. Recovery Situation</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Pick what is breaking down in the deal.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {RECOVERY_SITUATIONS.map((item) => (
              <ChipButton key={item} active={situation === item} label={item} onClick={() => withUsageTracking(() => setSituation(item))} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">2. Current Customer Status</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Timing changes the best recovery approach.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {CUSTOMER_STATUSES.map((item) => (
              <ChipButton key={item} active={customerStatus === item} label={item} onClick={() => withUsageTracking(() => setCustomerStatus(item))} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">3. Optional Sharpeners</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Mindset and goal refine tone and recovery path.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#84a0c4]">Customer Mindset</p>
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {CUSTOMER_MINDSETS.map((item) => (
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#84a0c4]">Recovery Goal</p>
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                {RECOVERY_GOALS.map((item) => (
                  <ChipButton key={item} active={recoveryGoal === item} label={item} onClick={() => withUsageTracking(() => setRecoveryGoal(item))} />
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Ex: customer reacted to trade value, then went quiet after numbers"
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Your Recovery Plan</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Likely Breakdown</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.likelyBreakdown}</p></CardContent>
          </Card>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Best Recovery Path</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.bestRecoveryPath}</p></CardContent>
          </Card>
          ...
          <Card className="border-[#233d58] bg-[#0f2238]">
            <CardHeader><CardTitle className="text-base text-[#8ad9ff]">Why This Works</CardTitle></CardHeader>
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Deeper diagnosis, sharper recovery angle, and delivery coaching.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">What is probably happening:</span> {sprocketOutput.probablyReallyHappening}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Better recovery angle:</span> {sprocketOutput.betterRecoveryAngle}</p>
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">If this fails, do this:</span> {sprocketOutput.fallbackNextStep}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware recovery personalization and focus guidance.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
            ...
            <CardContent className="space-y-3">
              ...
              {cxOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">Skill-aware adjustment:</span> {cxOutput.skillAwareAdjustment}</p>
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.situation} - {scenario.customerStatus}</p>
                    <Button
                      size="sm"
                      variant="ghost"
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks skill-aware recovery intelligence.' : undefined)}
        onOpenChange={(open) => {
