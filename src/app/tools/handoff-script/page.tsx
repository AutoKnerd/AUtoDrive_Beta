...
import {
  ...
  HANDOFF_SITUATIONS,
  HANDOFF_TYPES,
  getAutoDriveCxHandoffEnhancement,
  getHandoffPlan,
  getSprocketHandoffEnhancement,
  type HandoffInput,
  type HandoffMindset,
  type HandoffSavedScenario,
  type HandoffSituation,
  type HandoffType,
} from '@/lib/tools/handoff-script-optimizer';

const TOOL_ID = 'handoff-script';
const LOCAL_SCENARIOS_KEY = 'handoffScriptSavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): HandoffSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as HandoffSavedScenario[];
...
function writeLocalScenarios(scenarios: HandoffSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: HandoffInput, summary: string): string {
  return [
    'HANDOFF SCRIPT OPTIMIZER',
    '',
    `Handoff Type: ${input.handoffType}`,
    `Situation: ${input.situation}`,
    `Customer Mindset: ${input.customerMindset || 'neutral'}`,
...
export default function HandoffScriptPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [handoffType, setHandoffType] = useState<HandoffType>('TO manager (desk)');
  const [situation, setSituation] = useState<HandoffSituation>('normal progression (no tension)');
  const [customerMindset, setCustomerMindset] = useState<HandoffMindset>('neutral');
  const [context, setContext] = useState('');

  const [savedScenarios, setSavedScenarios] = useState<HandoffSavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketHandoffEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxHandoffEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [handoffType, situation, customerMindset, context]);

  const input = useMemo<HandoffInput>(() => ({
    handoffType,
    situation,
    customerMindset,
    context,
  ...
  const plan = useMemo(() => getHandoffPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Set Up Before Handoff] ${plan.setupBeforeHandoff}`,
      `[How to Frame the Handoff] ${plan.frameTheHandoff}`,
      `[Say This] ${plan.sayThis}`,
      `[Reinforce After Introduction] ${plan.reinforceAfterIntroduction}`,
    ...
    try {
      ...
      toast({ title: 'Copied', description: 'Handoff plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: HandoffSavedScenario = {
      id: crypto.randomUUID(),
      createdAt: new Date().toISOString(),
      handoffType,
      situation,
      customerMindset,
      setupBeforeHandoff: plan.setupBeforeHandoff,
      frameTheHandoff: plan.frameTheHandoff,
      sayThis: plan.sayThis,
      reinforceAfterIntroduction: plan.reinforceAfterIntroduction,
  ...
  }, [customerMindset, handoffType, plan.doNotDoThis, plan.frameTheHandoff, plan.reinforceAfterIntroduction, plan.sayThis, plan.setupBeforeHandoff, savedScenarios, situation, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync handoff scenarios.')) return;
    if (!firebaseUser) {
      toast({ variant: 'destructive', title: 'Sign in required', description: 'Sign in to save this handoff.' });
      return;
    }
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Say This: ${plan.sayThis}\nFrame: ${plan.frameTheHandoff}`),
    });
  ...
  }, [firebaseUser, input, plan.frameTheHandoff, plan.sayThis, requireFeature, toast]);
  ...
  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized handoff guidance.')) return;
    setCxOutput(getAutoDriveCxHandoffEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[HandoffScript] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Handoff Script Optimizer</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Build confident, natural handoffs to managers, finance, and specialists without losing momentum.
          </p>
          <p className="text-xs uppercase tracking-[0.12em] text-[#6f89af]">Smooth transitions, stronger trust, better deal continuity</p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">1. Handoff Type</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Pick the transition you need to execute.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {HANDOFF_TYPES.map((item) => (
              <ChipButton key={item} active={handoffType === item} label={item} onClick={() => withUsageTracking(() => setHandoffType(item))} />
            ))}
          </CardContent>
        </Card>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">2. Situation</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Set the moment so the handoff framing fits.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {HANDOFF_SITUATIONS.map((item) => (
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader>
            <CardTitle className="text-lg text-[#f2f7ff]">3. Customer Mindset (Optional)</CardTitle>
            <CardDescription className="text-[#9cb0cd]">Sharpen tone and control based on customer posture.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {HANDOFF_MINDSETS.map((item) => (
                <ChipButton key={item} active={customerMindset === item} label={item} onClick={() => withUsageTracking(() => setCustomerMindset(item))} />
              ))}
            </div>

            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#84a0c4]">Context notes (optional)</p>
              <Textarea
                value={context}
                onChange={(event) => withUsageTracking(() => setContext(event.target.value))}
                placeholder="Ex: trade involved, before manager TO, customer in a hurry"
        ...
        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Your Handoff Plan</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Set Up Before Handoff</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.setupBeforeHandoff}</p></CardContent>
          </Card>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">How to Frame the Handoff</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.frameTheHandoff}</p></CardContent>
          </Card>
          ...
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Reinforce After Introduction</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.reinforceAfterIntroduction}</p></CardContent>
          </Card>
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Diagnose handoff risk and sharpen positioning.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">What could go wrong:</span> {sprocketOutput.likelyFailureRisk}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Better positioning:</span> {sprocketOutput.betterPositioning}</p>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware handoff adaptation and coaching.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
            ...
            <CardContent className="space-y-3">
              ...
              {cxOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  ...
                  <p><span className="font-semibold text-[#88f3ff]">Skill-aware adjustment:</span> {cxOutput.adjustedFraming}</p>
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.handoffType} - {scenario.situation}</p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.sayThis}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks skill-aware transition intelligence.' : undefined)}
        onOpenChange={(open) => {
