...
import {
  COACHING_DEAL_STAGES,
  COACHING_OBSERVED_ISSUES,
  COACHING_REP_EXPERIENCE,
  COACHING_SEVERITIES,
  getAutoDriveCxTeamCoachingEnhancement,
  getSprocketTeamCoachingEnhancement,
  getTeamCoachingPlan,
  type CoachingDealStage,
  type CoachingObservedIssue,
  type CoachingRepExperience,
  type CoachingSeverity,
  type TeamCoachingInput,
  type TeamCoachingSavedScenario,
} from '@/lib/tools/team-coaching-converter';

const TOOL_ID = 'team-coaching-converter';
const LOCAL_SCENARIOS_KEY = 'teamCoachingConverterSavedScenariosV1';
const TOOLBOX_UPGRADE_URL = 'https://app.autodrivecx.com/signup';
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

type GateModalType = 'paid' | 'autodrive_cx' | null;

function readLocalScenarios(): TeamCoachingSavedScenario[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = localStorage.getItem(LOCAL_SCENARIOS_KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as TeamCoachingSavedScenario[];
...
function writeLocalScenarios(scenarios: TeamCoachingSavedScenario[]) {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_SCENARIOS_KEY, JSON.stringify(scenarios));
}

function buildCloudContent(input: TeamCoachingInput, summary: string): string {
  return [
    ...
    `Rep Experience: ${input.repExperience}`,
    '',
    summary,
  ].join('\n');
}

export default function TeamCoachingConverterPage() {
  const { toast } = useToast();
  const { user, firebaseUser } = useAuth();

  const [observedIssue, setObservedIssue] = useState<CoachingObservedIssue>('poor objection handling');
  const [dealStage, setDealStage] = useState<CoachingDealStage>('middle');
  const [severity, setSeverity] = useState<CoachingSeverity>('moderate');
  const [repExperience, setRepExperience] = useState<CoachingRepExperience>('developing');
  const [savedScenarios, setSavedScenarios] = useState<TeamCoachingSavedScenario[]>([]);
  ...
  const [sprocketOutput, setSprocketOutput] = useState<ReturnType<typeof getSprocketTeamCoachingEnhancement> | null>(null);
  const [cxOutput, setCxOutput] = useState<ReturnType<typeof getAutoDriveCxTeamCoachingEnhancement> | null>(null);
  ...
  useEffect(() => {
    ...
    setCxOutput(null);
  }, [observedIssue, dealStage, severity, repExperience]);

  const input = useMemo<TeamCoachingInput>(() => ({
    observedIssue,
    dealStage,
    severity,
    repExperience,
  }), [dealStage, observedIssue, repExperience, severity]);

  const plan = useMemo(() => getTeamCoachingPlan(input), [input]);
  ...
  const handleCopy = useCallback(async () => {
    const payload = [
      `[Likely Root Cause] ${plan.likelyRootCause}`,
      `[Coach-Back Message] ${plan.coachBackMessage}`,
      `[Do This Next Time] ${plan.doThisNextTime}`,
    ...
    try {
      ...
      toast({ title: 'Copied', description: 'Coaching plan copied.' });
    } catch {
  ...
  const handleSaveLocal = useCallback(() => {
    const scenario: TeamCoachingSavedScenario = {
      ...
      coachBackMessage: plan.coachBackMessage,
      doThisNextTime: plan.doThisNextTime,
  ...
  }, [dealStage, observedIssue, plan.coachBackMessage, plan.doNotSayThis, plan.doThisNextTime, plan.likelyRootCause, plan.practiceFocus, repExperience, savedScenarios, severity, toast]);

  const handleSaveCloud = useCallback(async () => {
    if (!requireFeature(FEATURES.CLOUD_SAVE, 'Unlock cloud saves to sync team coaching scenarios.')) return;
    if (!firebaseUser) {
    ...
    const result = await saveToolboxEntry({
      idToken,
      toolId: TOOL_ID,
      content: buildCloudContent(input, `Likely Root Cause: ${plan.likelyRootCause}\nDo This Next Time: ${plan.doThisNextTime}`),
    });
  ...
  }, [firebaseUser, input, plan.doThisNextTime, plan.likelyRootCause, requireFeature, toast]);

  const handleRunSprocket = useCallback(() => {
    if (!requireFeature(FEATURES.SPROCKET, 'Unlock Sprocket for deeper coaching conversion guidance.')) return;
    setSprocketOutput(getSprocketTeamCoachingEnhancement(input, plan));
  }, [input, plan, requireFeature]);

  const handleRunAutoDrive = useCallback(() => {
    if (!requireFeature(FEATURES.AUTODRIVE_CX, 'Upgrade to AutoDriveCX for personalized coaching adaptation.')) return;
    setCxOutput(getAutoDriveCxTeamCoachingEnhancement(input, plan, user));
  ...
  async function handleUnlockByEmail(values: { email: string; role: ToolboxCapturedRole }) {
    ...
    if (!captureResult.ok) {
      console.warn('[TeamCoachingConverter] unlock capture failed:', captureResult.message);
    }
    ...
    <div className="min-h-screen bg-[#070d18] text-[#dce7f8]">
      ...
      <main className="mx-auto w-full max-w-4xl space-y-5 px-4 pb-24 pt-4 sm:px-5 md:space-y-6 md:px-8 md:pt-8">
        ...
        <section className="space-y-2">
          <h1 className="text-2xl font-semibold tracking-tight text-[#f5f9ff] md:text-3xl">Team Coaching Converter</h1>
          <p className="max-w-2xl text-sm text-[#a7b7d1] md:text-base">
            Convert live deal observations into short, constructive, and actionable rep coaching.
          </p>
        </section>
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">1. Observed Issue</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {COACHING_OBSERVED_ISSUES.map((item) => (
              <ChipButton key={item} active={observedIssue === item} label={item} onClick={() => withUsageTracking(() => setObservedIssue(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">2. Deal Stage</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {COACHING_DEAL_STAGES.map((item) => (
        ...
        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">3. Severity</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-3 gap-2">
            {COACHING_SEVERITIES.map((item) => (
              <ChipButton key={item} active={severity === item} label={item} onClick={() => withUsageTracking(() => setSeverity(item))} />
            ))}
          </CardContent>
        </Card>

        <Card className="border-[#2b3e5d] bg-[#0f1b30]">
          <CardHeader><CardTitle className="text-lg text-[#f2f7ff]">4. Rep Experience</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-3 gap-2">
            {COACHING_REP_EXPERIENCE.map((item) => (
              <ChipButton key={item} active={repExperience === item} label={item} onClick={() => withUsageTracking(() => setRepExperience(item))} />
            ))}
          </CardContent>
        </Card>

        <section className="space-y-3">
          <h2 className="text-xl font-semibold text-[#f4f8ff]">Coaching Plan</h2>

          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Likely Root Cause</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.likelyRootCause}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Coach-Back Message</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.coachBackMessage}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Do This Next Time</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.doThisNextTime}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
            <CardHeader><CardTitle className="text-base text-[#7eeeff]">Practice Focus</CardTitle></CardHeader>
            <CardContent><p className="text-sm text-[#eff6ff]">{plan.practiceFocus}</p></CardContent>
          </Card>
          <Card className="border-[#2d4b66] bg-[#10243a]">
          ...
          fallback={(gate) => (
            <Card className="border-[#2f4568] bg-[#0f1c31]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Sharper coaching phrasing and manager delivery guidance.</CardDescription>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              {sprocketOutput && (
                <div className="space-y-2 rounded-xl border border-[#2e5872] bg-[#0c1d2f] p-3 text-sm text-[#dce9fb]">
                  <p><span className="font-semibold text-[#88f3ff]">Likely blind spot:</span> {sprocketOutput.likelyBlindSpot}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Better coaching phrasing:</span> {sprocketOutput.betterCoachingPhrasing}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Developmental rewrite:</span> {sprocketOutput.developmentalRewrite}</p>
                  <p><span className="font-semibold text-[#88f3ff]">Manager delivery coaching:</span> {sprocketOutput.managerDeliveryCoaching}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          fallback={(gate) => (
            <Card className="border-[#35556f] bg-[#101f33]">
              <CardHeader>
                ...
                <CardDescription className="text-[#9cb0cd]">Skill-aware coaching adaptation for manager style.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <p className="text-sm text-[#b8c8e2]">{gate.message}</p>
                <Button
              ...
              savedScenarios.slice(0, 6).map((scenario) => (
                <div key={scenario.id} className="rounded-xl border border-[#29415e] bg-[#0c182a] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-[#e8f1ff]">{scenario.observedIssue} - {scenario.severity}</p>
                    <Button
                      size="sm"
                      variant="ghost"
                  ...
                  <p className="text-sm text-[#c9d7ee]">{scenario.likelyRootCause}</p>
                </div>
              ))
            )}
          </CardContent>
      ...
      <UpgradeModal
        ...
        contextMessage={upgradeContextMessage || (gateModalType === 'autodrive_cx' ? 'AutoDriveCX unlocks personalized coaching adaptation.' : undefined)}
        onOpenChange={(open) => {
