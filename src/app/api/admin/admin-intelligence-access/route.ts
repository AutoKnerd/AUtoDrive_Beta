...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'revoke') {
      await targetRef.set({
        hasAdminIntelligenceAccess: false,
        adminIntelligenceAccessRevokedAt: nowIso,
        adminIntelligenceAccessRevokedBy: auth.actorId,
        updatedAt: nowIso,
      }, { merge: true });
    } else {
      await targetRef.set({
        hasAdminIntelligenceAccess: true,
        adminIntelligenceAccessGrantedAt: nowIso,
        adminIntelligenceAccessGrantedBy: auth.actorId,
        updatedAt: nowIso,
      }, { merge: true });
    }
    ...
    return NextResponse.json({
      ...
      user: {
        ...
        name: updated.name,
        email: updated.email,
        hasAdminIntelligenceAccess: Boolean(updated.hasAdminIntelligenceAccess),
      },
    }, { status: 200 });
  } catch (error: any) {
    return NextResponse.json({ message: error?.message || 'Failed to update admin intelligence access.' }, { status: 500 });
  }
}
