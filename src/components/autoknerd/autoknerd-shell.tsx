            ...
            <nav className="hidden h-full items-center space-x-8 md:flex">
              <div className="dropdown-group relative flex h-full items-center">
                ...
                <div className="dropdown-animate absolute left-0 top-full flex w-64 flex-col space-y-2 border border-zinc-800 bg-zinc-950 p-4 shadow-2xl">
                  {systemItems.map((item) => (
                    <Link key={item.title} className="group flex flex-col p-3 transition-colors hover:bg-zinc-900" href={item.href}>
                      <span className="mb-1 block text-xs font-bold uppercase tracking-widest text-lime-400">{item.title}</span>
                      <span className="block text-[10px] uppercase text-zinc-500">{item.subtitle}</span>
                    </Link>
                  ))}
                </div>
              </div>
