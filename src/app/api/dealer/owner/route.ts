...
import { getOwnerDashboardData } from '@/lib/dealer-dashboard';
...
export async function GET(req: Request) {
  try {
    ...
    const { user } = auth;
    const allowedRoles = new Set(['Owner', 'Admin', 'Developer']);
    if (!allowedRoles.has(user.role)) {
      return NextResponse.json({ message: 'Forbidden: Owner access required.' }, { status: 403 });
    }
    ...
    const data = await getOwnerDashboardData(dealershipId);
