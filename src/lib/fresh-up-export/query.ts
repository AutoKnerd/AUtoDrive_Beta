...
function normalizeSession(raw: Record<string, unknown>, docId: string): FreshUpNormalizedSession | null {
  ...
  return {
    ...
    isSandbox,
    sourceType,
    roleType: normalizeString(raw.roleType),
    interactionDisplayLabel: normalizeString(raw.interactionDisplayLabel),
    concernCategoryRoleSpecific: normalizeString(raw.concernCategoryRoleSpecific) || normalizeString(raw.primaryConcern),
    nextStepType: normalizeString(raw.nextStepType) || normalizeString(raw.recommendedNextStep),
    interpretationVersion: normalizeString(raw.roleLanguageVersion),
